...
// 确定对战圆顶训练家的战斗风格
#define MOVE_POINTS_COMBO      0   // 组合效果良好的招式：受天气/场地影响或能制造这些效果的招式、积食+、场地障碍、导致睡眠及从中受益的效果、提升单一属性至少两级或至少两个属性的攻击性增益，以及其他一些效果
#define MOVE_POINTS_STAT_RAISE 1   // 提升状态的招式
#define MOVE_POINTS_STAT_LOWER 2   // 降低状态的招式
#define MOVE_POINTS_RARE       3   // 出现率低于5%的升级学习列表中的招式
#define MOVE_POINTS_HEAL       4   // 治愈的招式
#define MOVE_POINTS_RISKY      5   // 被翡翠开发者认为是高风险的招式效果（出于某种原因，不包括高跳踢等）
#define MOVE_POINTS_STATUS     6   // 不造成伤害而引发状态效果的招式
#define MOVE_POINTS_DMG        7   // 造成伤害的招式（BP > 0）
#define MOVE_POINTS_DEF        8   // 像屏幕一样的招式、降低准确度或提升闪避率的招式、（特殊）防御提升招式、保护等
#define MOVE_POINTS_ACCURATE   9   // 准确度为100%的招式（或保证命中的招式）
#define MOVE_POINTS_POWERFUL   10  // 100 BP或以上的招式
#define MOVE_POINTS_POPULAR    11  // 90 BP或以上或提升单一攻击属性至少两级的TM/HM招式
#define MOVE_POINTS_LUCK       12  // 依赖运气的招式效果以及准确度小于或等于50%的招式
#define MOVE_POINTS_STRONG     13  // 90 BP或以上的招式
#define MOVE_POINTS_LOW_PP     14  // 5 PP或以下的招式
#define MOVE_POINTS_EFFECT     15  // 具有额外效果的招式
