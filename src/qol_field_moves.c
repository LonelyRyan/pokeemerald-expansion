...
static bool32 PartyCanLearnMoveLevelUp(u16 species, u16 moveId)
{
    ...
    for (i = 0; statsMovesLevelUp[i] != numLevelUpMoves; i++)
    {
        if ((statsMovesLevelUp[i] & numLevelUpMoves) == moveId)
            return TRUE;
    }
    return FALSE;
}
...
bool32 PartyHasMonLearnsKnowsFieldMove(u16 itemId)
{
    ...
    for (i = 0; i < PARTY_SIZE; i++)
    {
        ...
        if ((PartyCanLearnMoveLevelUp(species, moveId)|| (monCanLearnTM) == ALREADY_KNOWS_MOVE)|| (monCanLearnTM) == CAN_LEARN_MOVE)
